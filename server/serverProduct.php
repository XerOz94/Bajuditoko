...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    unset($input, $data, $data2, $id_product, $id_brand, $nama_product, $imageUrl, $stok, $harga, $deskripsi, $aksi, $abc);
} elseif($_SERVER['REQUEST_METHOD'] == 'GET') {
    if(($_GET['aksi'] == 'tampil') and (isset($_GET['id_product']))) {
        $id_product = filter($_GET['id_product']); // Ubah 'id product' menjadi 'id_product'
        $data = $abc->tampil_data($id_product); // Ubah 'tampil data' menjadi 'tampil_data'
        echo json_encode($data);
    } else { //menampilkan semua data
        $data = $abc->tampil_semua_data();
        echo json_encode($data);
    }
    unset($postdata, $data, $id_product, $abc);
}
